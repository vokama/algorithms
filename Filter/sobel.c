...
void sobel(char* name, char* newname)
{
	int i, j, Gx, Gy, G, **Intensity;
	char* A;
	...
	for(i = 2; i <= HeaderOfImage.HeightImage - 1; i++)
	{
		for(j = 2; j <= HeaderOfImage.WidthImage - 1; j++)
		{
			...
			Gy = abs( - Intensity[i + 1][j - 1] - 2 * Intensity[i + 1][j] - Intensity[i + 1][j + 1] + Intensity[i - 1][j - 1] + 2 * Intensity[i - 1][j] + Intensity[i - 1][j + 1]);
			G = sqrt(Gx * Gx + Gy * Gy);
			NewImage[i][j].B = G * 255 / 3060;
